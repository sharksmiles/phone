<template>
	<view class="u-page">
					...
					<u-col span="6">
						<view class="demo-layout bg-purple-light">
							<view style="display: flex;align-items: center;" class="game-conts">
								<image :src="game.selectedTraining.image_url" alt="">
									<span class="gameName">{{game.selectedTraining.name}}</span>
					...
					<u-grid :col="2" :border="false">
						<u-grid-item :custom-style="{padding: 0}" v-for="(item, index) in bannerList"
						 ...
						 @click="officialClick(item.id)" >
							<image class="image" :class="item.id == official_certificationId?'active':''" mode="widthFix" :src="item.src"></image>
						</u-grid-item>
					</u-grid>
				</view> -->
					...
					<view class="pweiwan viewFlex">
						<view class="">
							...
							<view style="line-height: 10px;">
								...
								<span style="color: #a8a8a8; font-size: 8px;">个代练正在待命</span>
							</view>
						</view>
						<view class="right">
							<view class="goto">
		...
		<training-list :list="list" @loadmore="loadmore" ref="list" />
...
<script>
	...
	export default {
		...
		methods: {
			gameClick(item) {
				this.$store.dispatch("game/selectedTrainingGame", item)
				this.getFilter()
				this.search()
			},
			search() {
			...
			getList() {
				...
				query.gameId = this.game.selectedTraining.id
				query.official_certificationId = that.official_certificationId
				getListApi(3, this.query).then(res => {
					const {
						list,
						total
					} = res.data
			...
			qqGroupClick() {
				...
				uni.showModal({
					title: "点单QQ群号",
					content: this.config.data.qqGroupTraining,
					showCancel: false,
					success: function(res) {
						if (res.confirm) {
							that.$utils.copy(that.config.data.qqGroupTraining)
						} else if (res.cancel) {
							console.log('用户点击取消');
						}
					}
