<template>
	<view class="loginWrap ">
		...
		<u-form class="loginForm" :model="form">
			...
			<block  v-if="loginWay == 2">
				...
				<view class="uni-form-item uni-column">
					...
					<view class="wrap">
						...
						@change="codeChange"
						></u-verification-code>
						<u-button type="success" style="background-color: #61c9f0;" size="mini" @click="getCode">{{tips}}</u-button>
					</view>
				</view>
			</block>
			...
			<p style="font-size: 14px; margin: 30px 15px;text-align: center;">
				<u-checkbox-group @change="checkboxGroupChange" shape="circle">
					<u-checkbox v-model="checked" active-color="red">
						我已阅读并同意
						<span style=" color: #61c9f0;text-decoration: underline;" @click="$utils.handleNavigate('/pages/common/xieyi?name=userAgreement')">
							《服务协议》
						</span>
					</u-checkbox>
				</u-checkbox-group>
			...
			<u-button :disabled="!checked" type="success" style="background-color: #61c9f0;" class="loginbtn" @click="formSubmit">登录</u-button>
		</u-form>
...
<style lang="less">
	...
	.loginWrap{
		...
		.loginForm{
			...
			.loginbtn{margin: 20upx; margin-top: 60upx;}
		}
		.action{
			padding: 10upx 30upx;
		}
